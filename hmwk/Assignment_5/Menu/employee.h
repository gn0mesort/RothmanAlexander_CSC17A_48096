...
public:
    ...
    /**
     * Thrown when an invalid Employee number is set. Invalid numbers are greater than 9999 or less than 0
     */
    class InvalidEmployeeNumber{
    };

    Employee(const std::string& = "", unsigned int = 0, const std::string& = "");
...
public:
    ...
    /**
     * Thrown when a shift value less than 1 or greater than 2 is set
     */
    class InvalidShift{
    };

    /**
     * Thrown when a pay rate of less than $0.01 is set
     */
    ...
    void shift(unsigned char nShift){
        if(nShift > 2 || nShift < 1){ //Check shift
            throw InvalidShift();
        }
        _shift = nShift;
    }
